<script>
// tem que colcoar script setup, se não tiver vai ter que exportar o js
...
//implementar esse código depois // Obtém os elementos const searchIcon =
document.querySelector('.search_icon1'); // Seleciona o ícone de busca const
searchInput = document.getElementById('searchInput'); // Seleciona o campo de
entrada // Função para realizar a busca function performSearch() { const query =
searchInput.value.trim(); // Captura o valor do input e remove espaços extras if
(query) { alert(`Buscando por: ${query}`); // Aqui você pode substituir o alert
por alguma lógica para realizar a busca (exemplo: API ou filtragem). } else {
alert('Por favor, insira um termo de busca!'); } } // Adiciona o evento de
clique no ícone de busca searchIcon.addEventListener('click', performSearch);
